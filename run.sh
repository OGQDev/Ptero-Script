...
os_check(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        if  [ "$dist_version" != "24.04" ] && [ "$dist_version" != "18.04" ]; then
            output "Versão não suportada do Ubuntu. Apenas Ubuntu 24.04 e 18.04 são suportados."
            exit 2
        fi
    elif [ "$lsb_dist" = "debian" ]; then
...
install_daemon() {
    ...
    if  [ "$lsb_dist" =  "ubuntu" ] ||  [ "$lsb_dist" =  "debian" ]; then
        ...
        curl -sL https://deb.nodesource.com/setup_12.x | sudo bash -
            if [ "$lsb_dist" =  "ubuntu" ] && [ "$dist_version" = "24.04" ]; then
